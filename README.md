...
**Drops what has closed, and admits what it cannot check.** A scholarship whose deadline has gone is not a scholarship, it is a history lesson — so every stated deadline is re-checked daily and anything past it drops off the main list, onto its own page with the month it is expected to reopen.

The honest limit: most university pages never publish a deadline at all. Those listings say "No closing date stated" rather than being dressed up as open, and the count above the results separates the ones with a date we re-check from the ones without. Claiming to have verified a date that was never published would be the same dishonesty as the sites this replaces, just in the other direction.
